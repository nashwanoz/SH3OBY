...
class AppRepository(private val context: Context) {
    ...
    suspend fun initialize() {
        ...
        db.withTransaction {
            val adminId = db.users().insert(
                UserEntity(username = "1", passwordHash = PasswordHasher.hash("1"), displayName = "المدير العام", role = "ADMIN")
            )
            db.warehouses().insert(WarehouseEntity(1, "المستودع الرئيسي", isMain = true))
            db.cashBoxes().insert(CashBoxEntity(1, "الصندوق الرئيسي"))
            seedProduct("ماء معدني", "629000000001", 1.0, "كرتون", 12, 10.0)
            seedProduct("عصير برتقال", "629000000002", 2.5, "كرتون", 24, 50.0)
            seedProduct("بسكويت", "629000000003", 1.5, "كرتون", 20, 25.0)
            seedProduct("مناديل ورقية", "629000000004", 3.0, "كرتون", 10, 27.0)
            check(adminId > 0)
        }
    }
    ...
    suspend fun login(username: String, password: String): UserEntity? =
        db.users().login(username.trim(), PasswordHasher.hash(password))
    ...
    suspend fun stockForUser(userId: Long): Flow<List<StockBalanceEntity>> {
        val warehouse = db.warehouses().forUser(userId) ?: return kotlinx.coroutines.flow.flowOf(emptyList())
        return db.stock().observeWarehouse(warehouse.id)
    }

    suspend fun stockForMain(): Flow<List<StockBalanceEntity>> {
        val warehouse = db.warehouses().main() ?: return kotlinx.coroutines.flow.flowOf(emptyList())
        return db.stock().observeWarehouse(warehouse.id)
    }

    suspend fun createUser(username: String, password: String, displayName: String): Long {
    ...
    suspend fun settle(adminId: Long, cashierId: Long, actual: Double): Result<Unit> = runCatching {
    ...
    private fun nextInvoiceId(userId: Long): String {
        val sequence = preferences.getLong("invoice_sequence_$userId", 0L) + 1
        preferences.edit().putLong("invoice_sequence_$userId", sequence).apply()
        return "$deviceId-$userId-$sequence"
    }
