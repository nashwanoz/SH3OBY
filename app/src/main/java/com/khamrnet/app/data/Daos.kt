...
@Dao
interface UserDao {
    @Query("SELECT * FROM users WHERE userCode = :userCode AND passwordHash = :passwordHash AND active = 1 LIMIT 1")
    suspend fun login(userCode: String, passwordHash: String): UserEntity?
